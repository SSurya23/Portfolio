    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
      {[...Array(16)].map((_, i) => (
        <div key={i} style={{
          ...
          borderRadius: '50%',
          background: i % 2 === 0 ? 'rgba(6,182,212,0.6)' : 'rgba(59,130,246,0.4)',
          left: `${10 + i * 5.5}%`, top: `${8 + i * 5}%`,
...
export default function Home({ setActive }) {
  ...
  useEffect(() => {
    let text = '', charIndex = 0, direction = 1, ri = 0;
    const interval = setInterval(() => {
      if (direction === 1) {
        text += ROLES[ri][charIndex];
        charIndex++;
        if (charIndex > ROLES[ri].length) { direction = -1; }
      } else {
        text = text.slice(0, -1);
        if (text.length === 0) {
          direction = 1;
      ...
      <div className="container" style={{
        ...
        <div style={{ animation: 'fadeInUp 0.8s ease forwards' }}>
          ...
          <p style={{
            ...
            marginBottom: 32, fontSize: 15, maxWidth: 460,
          }}>
            MCA graduate & M.Tech AI/ML student passionate about building scalable
            full-stack applications. Based in Kakinada, India — crafting digital
        ...
        <div className="animate-float" style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              background: 'radial-gradient(circle at 30% 30%, rgba(6,182,212,0.1), transparent 60%)',
            }} />
          </div>

          {/* Floating tech emojis */}
          {[['⚛️', '-8%', '10%'], ['☕', '85%', '20%'], ['🐍', '-6%', '55%'], ['🤖', '86%', '60%'], ['🗄️', '42%', '-5%']].map(([emoji, left, top], i) => (
            <div key={i} className="glass" style={{
              position: 'absolute', padding: '8px 10px', borderRadius: 10, fontSize: 18,
              left, top,
